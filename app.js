...
var mongoUrl = 'mongodb+srv://testuser:testuser@cluster0.2c95k.mongodb.net/lavanya?retryWrites=true&w=majority';
...
// first default route
app.get('/',(req,res) => {
    res.send("Hiii From Express Lavanya")
})
...
app.get('/Shop',(req,res) => {
    ...
    db.collection('products').find({"category":cat},{projection: { _id: 0 }}).toArray((err,result) => {
        if(err) throw err;
        res.send(result)
    })
    }else{
        db.collection('products').find({},{projection: { _id: 0 }}).toArray((err,result) => {
            if(err) throw err;
            res.send(result)
        })
    }
